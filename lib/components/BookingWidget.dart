...
class BookingWidgetState extends State<BookingWidget> {
  ...
  void init() async {
    ...
    if (sharedPref.getString(IS_TIME) == null) {
      ...
      if (duration > 0) {
        ...
        // if(sharedPref.getString("RIDE_ID_IS")==null||sharedPref.getString("RIDE_ID_IS")=="${widget.id}"){
        //   sharedPref.remove(IS_TIME);
        //   duration = timerMaxSeconds;
        //   return cancelRideCall();
        // }else{
        //   sharedPref.setString("RIDE_ID_IS","${widget.id}");
        // }
        sharedPref.remove(IS_TIME);
        duration = timerMaxSeconds;
        // return cancelRideCall();
  ...
  cancelRideCall(){
    ...
    rideRequestUpdate(request: req, rideId: widget.id).then((value) async {
      // await rideService.deleteRide(rideID: widget.id);
